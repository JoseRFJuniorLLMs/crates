...
fn append_concurrently(seg: u64, threads: usize, per: usize) -> Arc<Log> {
    ...
    let log = Arc::new(Log::open(dir.path(), seg, FsyncPolicy::GroupCommit { interval_ms: 50 }).unwrap());
    let hs: Vec<_> = (0..threads).map(|k| { let l = log.clone(); std::thread::spawn(move || {
        for i in 0..per { l.append(Episode::new("a", EventKind::Observation,
            format!("t{k}i{i}-{}", "x".repeat(120)).into_bytes())).unwrap(); }
    })}).collect();
...
fn scan_terminates_and_read_never_returns_wrong_lsn() {
    ...
    assert_eq!(wrong, 0, "read() devolveu {wrong} episódios com o LSN errado");
}
...
fn no_roll_no_loss() {
    ...
    let head = log.head();
    assert_eq!(log.scan(0, head).unwrap().len() as u64, head, "scan perdeu registos sem roll");
}
...
fn roll_must_not_lose_records() {
    ...
    let head = log.head();
    let missing = (0..head).filter(|&l| !matches!(log.read(l), Ok(Some(_)))).count();
...
fn stress_many_rolls_survives_reopen_and_verify() {
    ...
    {
        let log = Arc::new(Log::open(dir.path(), 4096, FsyncPolicy::GroupCommit { interval_ms: 50 }).unwrap());
        let hs: Vec<_> = (0..16usize).map(|k| { let l = log.clone(); std::thread::spawn(move || {
            for i in 0..100 {
                let n = 40 + (i * 7 + k * 13) % 400;
                l.append(Episode::new("a", EventKind::Observation,
                    format!("t{k}i{i}-{}", "x".repeat(n)).into_bytes())).unwrap();
            }})}).collect();
        for h in hs { h.join().unwrap(); }
        let head = log.head();
        assert!(log.sealed_segments().len() > 50, "o teste tem de provocar muitos rolls");
        assert_eq!(log.scan(0, head).unwrap().len() as u64, head, "scan perdeu registos");
        for l in 0..head {
            match log.read(l) {
        ...
        assert!(log.verify().is_ok(), "cadeia Merkle inconsistente após os rolls");
    }
    let log2 = Log::open(dir.path(), 4096, FsyncPolicy::Always).unwrap();
    let head2 = log2.head();
    assert_eq!(log2.scan(0, head2).unwrap().len() as u64, head2, "scan incompleto após reabrir");
}
