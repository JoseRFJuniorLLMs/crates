...
fn append_publico_recusa_o_kind_reservado() {
    ...
    let mau = Episode::new("atacante", EventKind::Custom("hvm_isa".into()), b"ola".to_vec());
    let r = eng.append(mau);
    assert!(r.is_err(), "o kind reservado hvm_isa foi aceite pelo append público");

    // O caminho H-VM legítimo continua a funcionar.
    eng.hvm_upsert(b"k".to_vec(), b"v".to_vec()).unwrap();
    let st = eng.hvm_state().unwrap();
    assert!(!st.memory_layers.is_empty(), "o upsert legítimo não entrou no ledger");
